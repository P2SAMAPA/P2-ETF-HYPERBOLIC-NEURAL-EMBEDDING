...
import data_manager
import push_results
from hne_engine import compute_hne_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (HNE Engine) ===")
        ...
        top_etfs    = [
            {"ticker": t, "hne_score": float(s), "best_window": int(w)}
        ...
        for win, score_dict in window_results.items():
            ...
            windows_tab2[str(win)] = {
                "top_etfs":    [{"ticker": t, "hne_score": float(s)} for t, s in sw[:config.TOP_N]],
    ...
    print(f"\n=== HNE Engine complete: {tab1_path.name}, {tab2_path.name} ===")
